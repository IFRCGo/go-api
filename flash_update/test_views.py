...
from flash_update.models import FlashEmailSubscriptions, FlashGraphicMap, FlashUpdate
...
class FlashUpdateTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.body = {
            "country_district": [
                {"country": str(self.country1.id), "district": [self.district1.id]},
                {"country": str(self.country2.id), "district": [self.district2.id]},
            ],
            "references": [
                {
                    "date": "2021-02-02",
            ...
            "actions_taken": [
                {"organization": "NTLS", "summary": "actions taken", "actions": [actions1.id, actions2.id]},
            ],
    ...
    def test_create_and_update(self, send_flash_update_email):
        ...
        # update
        data = self.body
        data["country_district"] = [{"country": str(self.country1.id), "district": [self.district1.id]}]
        data["references"] = [{"date": "2021-01-01", "source_description": "A source", "url": "https://youtube.com/"}]
        data["actions_taken"] = [
            {"organization": "NTLS", "summary": "actions taken updated", "actions": [self.actions3.id, self.actions4.id]},
            {"organization": "FDRN", "summary": "actions taken updated", "actions": [self.actions3.id, self.actions4.id]},
        ]
    ...
    def test_get_flash_update(self):
        ...
        self.assertEqual(
            sorted([flash_update1.id, flash_update2.id, flash_update3.id]), sorted([data["id"] for data in response1["results"]])
        )
    ...
    def test_validate_country_district(self):
        ...
        self.body["country_district"] = [{"country": str(self.country1.id), "district": str(self.district2.id)}]
        self.client.force_authenticate(user=self.user)
        with self.capture_on_commit_callbacks(execute=True):
    ...
    def test_send_email(self, send_notification):
        ...
        email_suscription = FlashEmailSubscriptions.objects.get(share_with=FlashUpdate.FlashShareWith.IFRC_SECRETARIAT)
        ...
        self.assertIn(
            email_data["actions_taken"][0]["id"], [data["id"] for data in instance.actions_taken_flash.all().values("id")]
        )
    ...
    def test_flash_update_share(self, send_notification, render_to_pdf):
        render_to_pdf.return_value = {"filename": "test.pdf", "file": b"pdf content"}
