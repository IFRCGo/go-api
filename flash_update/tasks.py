...
from .utils import get_email_context, render_to_pdf
...
def share_flash_update(flash_update_share_id):
    ...
    email_context = {"document_url": flash_update.extracted_file.url, "situational_overview": flash_update.situational_overview}
    donors_emails = instance.donors.all().values_list("email", flat=True)
    donor_groups_emails = Donors.objects.filter(groups__in=instance.donor_groups.all()).values_list("email", flat=True)
...
def send_flash_update_email(flash_update_id):
    ...
    users_emails = User.objects.filter(groups__flash_email_subscription__share_with=share_with_group).values_list(
        "email", flat=True
    )
    if users_emails:
        send_notification(
