...
from .tasks import send_flash_update_email, share_flash_update
...
class FlashActionsTakenSerializer(NestedUpdateMixin, NestedCreateMixin, serializers.ModelSerializer):
...
class FlashReferencesSerializer(serializers.ModelSerializer):
        ...
        if districts:
            for district in districts:
                if district.country != data["country"]:
                    raise serializers.ValidationError({"district": gettext("Different districts found for given country")})
        return data


class FlashUpdateSerializer(NestedUpdateMixin, NestedCreateMixin, serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        transaction.on_commit(lambda: send_flash_update_email.delay(flash_update.id))
        return flash_update
...
class ShareFlashUpdateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        transaction.on_commit(lambda: share_flash_update.delay(flash_update_share.id))
