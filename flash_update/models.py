...
class FlashUpdate(models.Model):
    ...
    hazard_type = models.ForeignKey(
        DisasterType, verbose_name=_("hazard type"), related_name="flash_update_hazard_type", null=True, on_delete=models.SET_NULL
    )
    title = models.CharField(max_length=300)
    situational_overview = HTMLField(verbose_name=_("Situational Overview"), blank=True, default="")

    # map/graphics
    map = models.ManyToManyField(FlashGraphicMap, blank=True, verbose_name=_("map"), related_name="flash_map")
    graphics = models.ManyToManyField(FlashGraphicMap, blank=True, verbose_name=_("graphics"), related_name="flash_graphics")
    ...
    share_with = models.CharField(
        max_length=50,
        choices=FlashShareWith.choices,
        default=FlashShareWith.IFRC_SECRETARIAT,
        null=True,
        blank=True,
...
class FlashCountryDistrict(models.Model):
    flash_update = models.ForeignKey(
        FlashUpdate, on_delete=models.CASCADE, verbose_name=_("Flash update"), related_name="flash_country_district"
    )
    country = models.ForeignKey(Country, verbose_name=_("country"), on_delete=models.CASCADE, related_name="flash_country")
    district = models.ManyToManyField(District, verbose_name=_("district"), related_name="flash_district", blank=True)
    client_id = models.CharField(max_length=50, null=True, blank=True)
...
class FlashAction(models.Model):
    ...
    flash_update_types = ArrayField(
        ...
        verbose_name=_("flash update types"),
        default=list,
    )
...
class FlashEmailSubscriptions(models.Model):
    share_with = models.CharField(
        max_length=50,
        choices=FlashUpdate.FlashShareWith.choices,
