...
from .models import DomainWhitelist, Pending, Recovery, UserExternalToken
from .utils import is_valid_domain, jwt_encode_handler
    ...
    def save(self):
        ...
        try:
            user = User.objects.create_user(
                username=username, first_name=first_name, last_name=last_name, email=email, password=password, is_active=False
            )
        except Exception:
        ...
        pending = Pending(user=user, justification=justification, token=get_random_string(length=32))
        if not is_staff:
            pending.admin_token_1 = get_random_string(length=32)
        pending.save()

        # send notification
        transaction.on_commit(lambda: send_notification_create.delay(pending.token, username, is_staff, email))
...
class UserExternalTokenSerializer(serializers.ModelSerializer):
    ...
    is_old_token = serializers.BooleanField(required=False)
