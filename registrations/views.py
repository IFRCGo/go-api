...
class VerifyEmail(APIView):
    def get(self, request):
        ...
        if pending_user.email_verified:
            return bad_http_request(
                "You have already verified your email", "A validation email has been sent to the administrators you listed."
            )
        ...
        if is_valid_domain(pending_user.user.email):
            ...
            for admin in admins:
                ...
                send_notification(
                    "Reference to approve an account",
                    [admin],
...
class ValidateUser(APIView):
    def get(self, request):
        ...
        if not token or not user:
            return bad_http_request(
                "Credentials not found", "The URL must include a token and user. Please check your email and try again."
            )
        try:
            pending_user = Pending.objects.get(Q(admin_token_1=token) | Q(admin_token_2=token), user__username=user)
        except ObjectDoesNotExist:
            return bad_http_request(
                "User not found, or token incorrect", "We could not find a user and token that matched those supplied."
            )
        ...
        if did_validate:
            return bad_http_request("Already confirmed", "You have already confirmed this user.")
