...
class RelatedAppealSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = Appeal
        fields = ('aid', 'num_beneficiaries', 'amount_requested', 'amount_funded', 'id',)

class KeyFigureSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = KeyFigure
...
class SituationReportSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
...
class AppealDocumentSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
...
class FieldReportContactSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = FieldReportContact
        fields = ('ctype', 'name', 'title', 'email', 'id',)

class ActionsTakenSerializer(serializers.HyperlinkedModelSerializer):
...
class ListFieldReportSerializer(serializers.HyperlinkedModelSerializer):
    event = ListEventSerializer()
    class Meta:
        model = FieldReport
        fields = ('created_at', 'summary', 'event', 'dtype', 'countries', 'id',)

class DetailFieldReportSerializer(serializers.HyperlinkedModelSerializer):
    user = UserSerializer()
