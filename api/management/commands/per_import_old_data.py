...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-t", "--type", type=str, help="Type must be one of the following: atypes, areas, components, questions"
        )
        parser.add_argument("-a", "--areanum", type=str, help="Area number must be an integer")
                            ...
                            if comp:
                                ...
                                if last_question:
                                    bench_q = FormQuestion.objects.create(
                                        component_id=comp.id, question="Component performance", is_benchmark=True
                                    )
                                    bench_q.answers.add(
                                    ...
                                    epi_q = FormQuestion.objects.create(
                                        component_id=comp.id, question="Component Epidemic Preparedness", is_epi=True
                                    )
                                    epi_q.answers.add(
                                        not_reviewed, does_not_exist, partially, need_improv, exist, high_performance
                                    )
