...
class Command(BaseCommand):
    ...
    def construct_template_record(self, rtype, record):
        ...
        if rtype == RecordType.FIELD_REPORT:
            contacts = [('<strong>' + c.ctype + ' ·</strong> ' if c.ctype else '') +
                        '<strong>' + c.name + '</strong>' +
                        (', ' + c.title if c.title else '') +
                        (', <a href=mailto:"' + c.email + '">' + c.email + '</a>' if c.email else '') +
            ...
            rec_obj = {
                ...
                'other_fields': {
                    ...
                    'epi_notes_since_last_fr': record.epi_notes_since_last_fr,
                    ...
                    'affected_pop_centres': record.affected_pop_centres,
                    'gov_affected_pop_centres': record.gov_affected_pop_centres,
                    'gov_num_highest_risk': record.gov_num_highest_risk,
                    'gov_num_potentially_affected': record.gov_num_potentially_affected,
                    'health_min_num_assisted': record.health_min_num_assisted,
                    'who_num_assisted': record.who_num_assisted,
                    'ifrc_staff': record.ifrc_staff,
                    'imminent_dref': record.imminent_dref,
                    'imminent_dref_amount': record.imminent_dref_amount,
                    'is_covid_report': record.is_covid_report,
                    'notes_health': record.notes_health,
                    'notes_ns': record.notes_ns,
                    'notes_socioeco': record.notes_socioeco,
                    'num_expats_delegates': record.num_expats_delegates,
                    'num_fact': record.num_fact,
                    'num_highest_risk': record.num_highest_risk,
                    'num_ifrc_staff': record.num_ifrc_staff,
                    'num_localstaff': record.num_localstaff,
                    'num_potentially_affected': record.num_potentially_affected,
                    'num_rdrt': record.num_rdrt,
                    'num_volunteers': record.num_volunteers,
                    'other_affected_pop_centres': record.other_affected_pop_centres,
                    'other_num_highest_risk': record.other_num_highest_risk,
                    'other_num_potentially_affected': record.other_num_potentially_affected,
                    'other_sources': record.other_sources.split("\n"),
                    'rdrt': record.rdrt,
                    'regions': ', '.join(i.label for i in record.regions.all()),
