...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        partners = ["MOH", "WHO", "UNICEF", "WFP", "Other UN agencies", "Civil Society and NGO partners"]
        activities = [
            "COVID-19 testing",
        ...
        if partners_empty:
            for par in partners:
                extpar = ExternalPartner(name=par, name_en=par)
        ...
        if activities_empty:
            for act in activities:
                supact = SupportedActivity(name=act, name_en=act)
