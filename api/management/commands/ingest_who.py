...
#
                ...
                if country == 'Democratic Republic of the Congo': #replacement
                    ...
                    haccategory = data['category'].decode("utf-8") # for 'hac' category. See link for 'hac'
                    end = haccategory.find('[country]')
                    if end > 0:
                        start = haccategory[:end-1].rfind(',', 0) # backwards search the comma
                        country = haccategory[ start+2 : end-1 ] # Getting the comma followed part from the category as Country
                    else:
