...
from api.scrapers.config import S_KEYS, SF_KEYS  # M_EXTRACTORS,; get_sector_misc_keys,
                ...
                for sector_key in S_KEYS[sector]:
                    for field in SF_KEYS:
                        for field_key in SF_KEYS[field]:
                            search = re.search("{} {}".format(sector_key, field_key), text, re.IGNORECASE)
                            if search:
            ...
            for field, option in fields.items():
                ...
                if not self.extracted.get(sector).get(field):
                    self.extracted[sector][field] = {
                        "value": text,
                        "score": "{} {}".format(option["sector_score"] / 100, option["score"] / 100),
                    }
