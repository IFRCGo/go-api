...
class Region(models.Model):
    ...
    def get_national_society_count(self):
        return (
            Country.objects.filter(region=self, record_type=CountryType.COUNTRY, independent=True)
...
class Country(models.Model):
    ...
    record_type = models.IntegerField(
        choices=CountryType.choices, verbose_name=_("type"), default=1, help_text=_("Type of entity")
    )
    ...
    sovereign_state = models.ForeignKey(
        "self",
        verbose_name=_("Country ID of the Sovereign State"),
        null=True,
        blank=True,
        default=None,
    ...
    average_household_size = models.DecimalField(
        verbose_name=_("Average Household Size"), null=True, blank=True, max_digits=5, decimal_places=2
    )
    address_1 = models.CharField(verbose_name=_("Address 1"), null=True, blank=True, max_length=255)
    address_2 = models.CharField(verbose_name=_("Address 2"), null=True, blank=True, max_length=255)
    city_code = models.CharField(verbose_name=_("City Code"), null=True, blank=True, max_length=255)
    phone = models.CharField(verbose_name=_("Phone"), null=True, blank=True, max_length=255)
    website = models.CharField(verbose_name=_("Website"), null=True, blank=True, max_length=255)
    emails = ArrayField(
        models.CharField(
    ...
    founded_date = models.DateField(verbose_name=_("Found date"), null=True, blank=True)

    # disaster law url
    disaster_law_url = models.CharField(verbose_name=_("Disaster law url"), null=True, blank=True, max_length=255)
...
class District(models.Model):
    ...
    centroid = models.PointField(srid=4326, blank=True, null=True)
    bbox = models.PolygonField(srid=4326, blank=True, null=True)
    is_deprecated = models.BooleanField(default=False, help_text=_("Is this an active, valid district?"))
    # Population Data From WB API
    wb_population = models.PositiveIntegerField(
        verbose_name=_("WB population"), null=True, blank=True, help_text=_("population data from WB API")
    )
...
class CountrySupportingPartner(models.Model):
    ...
    last_name = models.CharField(verbose_name=_("Last Name"), max_length=255, null=True, blank=True)
    position = models.CharField(verbose_name=_("Position"), max_length=255, null=True, blank=True)
    email = models.CharField(verbose_name=_("Email"), max_length=255, null=True, blank=True)
    supporting_type = models.IntegerField(
        choices=SupportingPartnerType.choices, verbose_name=_("Supporting Type"), null=True, blank=True
    )
...
class CountryKeyDocument(models.Model):
    ...
    thumbnail = models.CharField(verbose_name=_("Thumbnail"), max_length=255)
    document_type = models.CharField(verbose_name=_("Document Type"), max_length=255)
...
class CountryCapacityStrengthening(models.Model):
    ...
    country = models.ForeignKey(Country, verbose_name=_("Country"), on_delete=models.CASCADE)
    assessment_code = models.CharField(verbose_name=_("Assessment Code"), max_length=255)
    year = models.CharField(verbose_name=_("Year of Assessment"), max_length=25)
...
class CountryOrganizationalCapacity(models.Model):
    ...
    country = models.OneToOneField(Country, verbose_name=_("Country"), on_delete=models.CASCADE)
    leadership_capacity = models.TextField(verbose_name=_("Leadership Capacity"), null=True, blank=True)
    youth_capacity = models.TextField(verbose_name=_("Youth Capacity"), null=True, blank=True)
    volunteer_capacity = models.TextField(verbose_name=_("Volunteer Capacity"), null=True, blank=True)
...
class CountryICRCPresence(models.Model):
    country = models.OneToOneField(Country, verbose_name=_("Country"), on_delete=models.CASCADE)
    icrc_presence = models.BooleanField(verbose_name=_("ICRC Presence"), null=True, blank=True)
    url = models.CharField(verbose_name=_("Url"), null=True, blank=True, max_length=255)
    key_operation = models.BooleanField(verbose_name=_("Key Operations"), null=True, blank=True)
...
class DistrictGeoms(models.Model):
    ...
    geom = models.MultiPolygonField(srid=4326, blank=True, null=True)
    district = models.OneToOneField(District, verbose_name=_("district"), on_delete=models.CASCADE, primary_key=True)
...
class RegionSnippet(models.Model):
    ...
    def __str__(self):
        return (self.snippet[:80] if self.snippet else None) or (
            self.image.url if getattr(self, "image", None) and self.image.url else ""
        )
...
class Event(models.Model):
    ...
    parent_event = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name=_("Parent Emergency"),
        on_delete=models.SET_NULL,
        help_text=_(
...
class EventFeaturedDocument(models.Model):
    ...
    file = models.FileField(
        verbose_name=_("file"),
        upload_to="event-featured-documents/file/",
    )
...
class SituationReport(models.Model):
    ...
    visibility = models.IntegerField(
        choices=VisibilityChoices.choices, verbose_name=_("visibility"), default=VisibilityChoices.MEMBERSHIP
    )
...
class Appeal(models.Model):
    ...
    def indexing(self):
        return {
            ...
            "body": "%s %s" % (self.name, getattr(self.country, "name", None)),
...
class AppealDocument(models.Model):
    ...
    iso = models.ForeignKey(
        Country, to_field="iso", db_column="iso", verbose_name=_("location"), null=True, on_delete=models.SET_NULL
    )
...
class FieldReport(models.Model):
    ...
    who_num_assisted = models.IntegerField(
        verbose_name=_("number of assisted (who)"), null=True, blank=True, help_text=_("not used any more")
    )
    health_min_num_assisted = models.IntegerField(
        verbose_name=_("number of assisted (ministry of health)"), null=True, blank=True, help_text=_("not used any more")
    )

    # Early Warning fields
    gov_num_potentially_affected = models.IntegerField(verbose_name=_("potentially affected (government)"), null=True, blank=True)
    gov_num_highest_risk = models.IntegerField(verbose_name=_("people at highest risk (government)"), null=True, blank=True)
    gov_affected_pop_centres = models.CharField(
        verbose_name=_("affected population centres (government)"), max_length=512, blank=True, null=True
    )
    ...
    appeal = models.IntegerField(choices=RequestChoices.choices, verbose_name=_("appeal"), default=0, null=True)
    appeal_amount = models.IntegerField(verbose_name=_("appeal amount"), null=True, blank=True)
    imminent_dref = models.IntegerField(
        choices=RequestChoices.choices, verbose_name=_("imminent dref"), default=0, null=True
    )  # only EW
    ...
    forecast_based_action = models.IntegerField(
        choices=RequestChoices.choices, verbose_name=_("forecast based action"), default=0, null=True
    )  # only EW
    forecast_based_action_amount = models.IntegerField(
        verbose_name=_("forecast based action amount"), null=True, blank=True
    )  # only EW
    ...
    eru_basic_health_care = models.IntegerField(
        choices=RequestChoices.choices, verbose_name=_("ERU basic health care"), default=0
    )
    ...
    eru_deployment_hospital = models.IntegerField(
        choices=RequestChoices.choices, verbose_name=_("ERU deployment hospital"), default=0
    )
    ...
    eru_referral_hospital = models.IntegerField(
        choices=RequestChoices.choices, verbose_name=_("ERU referral hospital"), default=0
    )
    ...
    eru_water_sanitation_15 = models.IntegerField(
        choices=RequestChoices.choices, verbose_name=_("ERU water sanitation M15"), default=0
    )
    ...
    eru_water_sanitation_40 = models.IntegerField(
        choices=RequestChoices.choices, verbose_name=_("ERU water sanitation M40"), default=0
    )
    ...
    eru_water_sanitation_20 = models.IntegerField(
        choices=RequestChoices.choices, verbose_name=_("ERU water sanitation MSM20"), default=0
    )
    ...
    external_partners = models.ManyToManyField(ExternalPartner, verbose_name=_("external partners"), blank=True)
    supported_activities = models.ManyToManyField(SupportedActivity, verbose_name=_("supported activities"), blank=True)
    ...
    def indexing(self):
        ...
        return {
            ...
            "visibility": self.visibility,
            "ns": " ".join(map(str, ns)) if len(ns) else None,
            "body": "%s %s" % (self.summary, " ".join(map(str, countries)) if len(countries) else None),
    ...
    def get_for(user, queryset=None):
        ...
        return queryset.exclude(Q(visibility=VisibilityChoices.IFRC_NS) & ~Q(countries__in=countries_qs))
...
class Action(models.Model):
    ...
    organizations = ArrayField(
        models.CharField(choices=ActionOrg.choices, max_length=4), verbose_name=_("organizations"), default=list, blank=True
    )
    field_report_types = ArrayField(
        models.CharField(choices=ActionType.choices, max_length=16),
        verbose_name=_("field report types"),
        default=list,
    )
...
class Profile(models.Model):
    ...
    org_type = models.CharField(
        choices=OrgTypes.choices, default=OrgTypes.OTHR, verbose_name=_("organization type"), max_length=4, blank=True
    )
    ...
    def sync_cron(body):
        ...
        if not len(errors):
            CronJob.objects.filter(
                name=body["name"], created_at__lt=datetime.now(pytz.timezone("UTC")) - timedelta(days=store_me)
...
class ReversionDifferenceLog(models.Model):
    ...
    changed_from = ArrayField(
        models.TextField(null=True, blank=True), verbose_name=_("changed from"), default=list, null=True, blank=True
    )
    changed_to = ArrayField(
        models.TextField(null=True, blank=True), verbose_name=_("changed to"), default=list, null=True, blank=True
    )
...
class ERPGUID(models.Model):
    ...
    api_guid = models.CharField(
        max_length=200, help_text="Can be used to do a GET request to check on the microservice API side."
    )
...
class Export(models.Model):
    ...
    status = models.IntegerField(verbose_name=_("Status"), choices=ExportStatus.choices, default=ExportStatus.PENDING)
    ...
    per_country = models.IntegerField(verbose_name=_("Per Country Id"), null=True, blank=True)
