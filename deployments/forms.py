...
from api.models import Country, DisasterType, District, VisibilityCharChoices
...
class ProjectImportForm(forms.Form):
    ...
    def generate_template(cls):
        country_districts = list(District.objects.values_list("country__name", "name").order_by("country__name", "name"))
    ...
    def _handle_bulk_upload(self, user, file, delimiter, quotechar):
        ...
        for row_number, row in enumerate(reader, start=2):
            district_names = (
                list({d.strip() for d in filter(lambda x: x.strip() != "", row[c.DISTRICT].split(","))})
                if row[c.DISTRICT].lower() not in ["countrywide", ""]
                else []
            )
            ...
            tag_names = list({d.strip() for d in filter(lambda x: x.strip() != "", row[c.TAGS].split(","))})
    ...
    def handle_bulk_upload(self, request):
        ...
        try:
            ...
            project_import.message = (
                f"Importing <b>{file}</b> failed. Check file and try again!!<br />" f"<pre>{traceback.format_exc()}</pre>"
            )
