...
def dict_to_string(dict_obj):
    return "-".join([str(dict_obj[key]) for key in sorted(dict_obj.keys())])
...
class ProjectGetTest(APITestCase):
    ...
    def test_visibility_project_get(self):
        ...
        assert public_project.pk in p_ids and private_project.pk not in p_ids and ifrc_only_project.pk not in p_ids
        # Normal user
        self.authenticate(self.user)
        resp = self.client.get(f"/api/v2/project/?project_country={country.pk}")
        p_ids = [p["id"] for p in resp.json()["results"]]
        assert public_project.pk in p_ids and private_project.pk in p_ids and ifrc_only_project.pk not in p_ids
        # IFRC user
