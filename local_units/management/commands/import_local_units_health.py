...
from local_units.models import HealthData, LocalUnit, LocalUnitType
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(filename) as csvfile:
            ...
            # Prefetch
            country_name_id_map = {country_name.lower(): _id for _id, country_name in Country.objects.values_list("id", "name")}
            local_unit_id_map = {code: _id for _id, code in LocalUnitType.objects.values_list("id", "code")}

            for i, row in enumerate(reader, start=2):
                # Without positions we can't use the row:
                if not row["LONGITUDE"] or not row["LATITUDE"]:
                    self.stdout.write(self.style.WARNING(f"Skipping row {i}: Empty locations point"))
                    continue
                if not row["NAME_LOC"] and not row["NAME_EN"]:
                    self.stdout.write(self.style.WARNING(f"Skipping row {i}: Empty brach name combination"))
                    continue
