...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(filename) as csvfile:
            ...
            # Prefetch dataset
            country_iso3_id_map = {
                iso3.lower(): _id for _id, iso3 in Country.objects.filter(iso3__isnull=False).values_list("id", "iso3")
            }
            dotype_id_map = {name: _id for _id, name in DelegationOfficeType.objects.values_list("id", "name")}
