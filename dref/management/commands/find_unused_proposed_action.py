...
def _through_table_and_column_names():
    """Return (table_names, fk_column_name) for the M2M 'proposed_action' relations."""
    tables = []
    fk_col = "proposedaction_id"
    for model in (Dref,):
        tables.append(model.proposed_action.through._meta.db_table)
    if DrefFinalReport is not None and hasattr(DrefFinalReport, "proposed_action"):
        tables.append(DrefFinalReport.proposed_action.through._meta.db_table)
...
WITH
    ...
    total_actions AS (SELECT COUNT(*) AS c FROM dref_proposedaction)
SELECT
...
class Command(BaseCommand):
    help = "List ProposedAction rows not linked via any 'proposed_action' M2M field (dangling)."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--move-danglings",
            action="store_true",
            help="Archive dangling rows to tmp_dref_proposedaction then delete originals.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if options.get("count"):
            self.stdout.write(f"Dangling ProposedAction count: {len(rows)}")
            return

        if not rows:
            self.stdout.write("No dangling ProposedAction records found.")
            with connection.cursor() as cursor:
        ...
        if options.get("verbose"):
            self.stdout.write(f"Found {len(rows)} dangling ProposedAction record(s):")
            for pk in rows:
    ...
    def _move_danglings(self, dangling_ids):
        if not dangling_ids:
            ...
            return
        self.stdout.write("\n-- Moving dangling rows to tmp_dref_proposedaction --")
        id_list_sql = ",".join(str(i) for i in dangling_ids)
        with connection.cursor() as cursor:
            cursor.execute(
                """
                CREATE TABLE IF NOT EXISTS tmp_dref_proposedaction (LIKE dref_proposedaction INCLUDING ALL);
                TRUNCATE TABLE tmp_dref_proposedaction;
                INSERT INTO tmp_dref_proposedaction SELECT * FROM dref_proposedaction WHERE id IN ("""
                + id_list_sql
                + ");"
            )
            cursor.execute("DELETE FROM dref_proposedaction WHERE id IN (" + id_list_sql + ")")
        self.stdout.write(f"Moved {len(dangling_ids)} rows to tmp_dref_proposedaction and deleted originals.")
    ...
    def _print_stats(self, stats_row, dangling_count=None):
        ...
        self.stdout.write("\n=== ProposedAction Usage Stats ===")
        self.stdout.write(f"Total proposed actions: {total_actions}")
